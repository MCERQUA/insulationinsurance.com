...
import { ShieldCheck, HardHat, Truck, Wrench, FileCheck, Umbrella, Zap, Wind, CheckCircle2, ArrowRight, ArrowLeft, Users, Target } from "lucide-react";
      ...
      <main>
          ...
          <div className="container-wide">
            ...
            <div className="grid lg:grid-cols-12 gap-10 items-center">
              <FadeIn className="lg:col-span-7">
                ...
                {detail && <p className="mt-5 lead max-w-2xl">{detail.intro}</p>}
              ...
              <FadeIn direction="left" className="lg:col-span-5">
                <div className="rounded-t-[10rem] rounded-b-3xl overflow-hidden border-4 border-white shadow-warm-lg">
                  <img src={`/images/${slug}.jpg`} alt={`${service.title} — insulation contractor insurance`} className="w-full h-[300px] md:h-[360px] object-cover" loading="lazy" />
                </div>
              </FadeIn>
            </div>
          </div>
        ...
        {detail && (
          <section className="bg-cream py-16 md:py-24">
            <div className="container-wide">
              <div className="prose-warm max-w-4xl mx-auto">
                <ul className="grid sm:grid-cols-2 gap-4 not-prose">
                  {detail.coveragePoints.map((c) => (
                    <li key={c} className="flex items-start gap-2.5 text-cocoa text-[0.95rem] leading-relaxed bg-white rounded-2xl border border-adobe p-4 shadow-card">
                      <CheckCircle2 className="h-5 w-5 text-clay flex-shrink-0 mt-0.5" /><span>{c}</span>
                    </li>
                  ))}
                </ul>
              </div>
